...
Because the coefficients and boundaries are independent of $x$, a Fourier mode evolves independently in the linear theory.  For a nonzero mode
\begin{equation}
    \phi_j(x,z,t)=\widehat{\phi}_{j}(z,t)e^{ikx},
    \qquad k>0,
\end{equation}
Laplace's equation \eqref{eq:laplace_potential} reduces to
\begin{equation}
    \frac{d^2\widehat{\phi}_j}{dz^2}-k^2\widehat{\phi}_j=0.
\end{equation}
The finite-depth wall conditions \eqref{eq:potential_wall_conditions} select hyperbolic functions.  Writing the modal interfacial velocity as
\begin{equation}
...
\end{align}
These shapes equal $\widehat{W}$ at the interface and vanish at the rigid walls.  Integrating once in $z$ gives the interface values of the velocity potentials,
\begin{equation}
    \widehat{\phi}_1(0,t)
    =
...
\end{equation}
The factors $\rho_1\coth(kH_1)$ and $\rho_2\coth(kH_2)$ that appear in the dispersion relation come from these interface potential values.  They represent the finite-depth inertial loading of the two fluids on the moving interface.
